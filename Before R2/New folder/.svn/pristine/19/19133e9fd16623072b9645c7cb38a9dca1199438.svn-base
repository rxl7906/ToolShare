...
def main(request):
	...
	rating = community.get_rating()
	user_array = UserProfile.objects.filter(community=community, user__is_active=True)
	
	return render(request, 'main.html', { 'community': str(community), 'rating': rating, 'user_array': user_array })
...
# allows a user to rate his/her community
@login_required
def rate(request):
	...
	if request.method == 'GET':
		...
		if form.is_valid():
			# saves the rating
			form.add_rating(request)
			
		return redirect('main')
		
	message = 'Add your rating of the community.'
	return render(request, 'rate.html', {'title': title, 'message': message, 'form': form})
